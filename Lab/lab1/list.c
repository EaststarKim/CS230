...
{
	for(;head->key!=search_key;head=head->next);
	return head;
}
...
{
	int cnt=0;
	for(head=head->next;head->key!=-1;head=head->next)++cnt;//init:excluding head, condition:excluding tail
	return cnt;
}
...
{
	return head->next->key==-1;// head - (empty) - tail
}
...
{
	for(head=head->next;head->key!=-1;head=head->next)printf("%d - ",head->key);
}
...
{
	for(head=head->next;head->key!=-1;head=head->next)if(head->key>=new_node->key)break;
	insert_node_after(head->prev,new_node);//head->prev->key < new_node->key <= head->key
}
